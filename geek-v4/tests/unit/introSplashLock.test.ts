...
describe('IntroAnimation の主要定数 ↔ splash 寸法/タイミング 一致', () => {
  ...
  it('退場はバーが左→右を完走してから (exit を sweep にアンカー / SWEEPS_BEFORE_EXIT≥1)', () => {
    // 「Geek の下のバーが左→右を完走するまで必ず表示」をコードで保証する回帰テスト。
    // 退場(fade 開始)が SWEEPS_BEFORE_EXIT × SWEEP_MS で計算されること = バーが必ず右端へ
    // 到達した瞬間に一致する。1 未満にすると左→右を見せ切れず「短すぎ違和感」が再発する。
    const m = introSrc.match(/SWEEPS_BEFORE_EXIT:\s*(\d+)/);
    expect(m).toBeTruthy();
    expect(Number(m![1])).toBeGreaterThanOrEqual(1);
    // exit / safety timer が sweep にアンカーされている (積で計算)
    expect(introSrc).toMatch(/SWEEPS_BEFORE_EXIT\s*\*\s*CFG\.SWEEP_MS/);
    // ★ 旧実装 (sweep 非依存の FADE_IN + HOLD アンカー) に戻っていないこと = ブツ切り再発防止
    expect(introSrc).not.toMatch(/CFG\.FADE_IN\s*\+\s*CFG\.HOLD/);
    expect(introSrc).not.toMatch(/HOLD:\s*\d+/);
  });
