...
export default function CalendarScreen() {
  ...
  // web のみ: ソフトキーボード高さ (native は常に 0 — KeyboardAvoidingView が担当)。
  // RNW の KeyboardAvoidingView は no-op なので、この inset を scrim 下 padding に
  // 足して bottom sheet 全体をキーボードの上へ持ち上げる。
    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      >
        ...
        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          <Text style={[T.h3, { color: C.text }]}>カレンダー</Text>
      ...
      >
        {isLoading ? (
          ...
          <EmptyState
            icon={Icon.calendar}
            title="イベントはまだありません"
            message={isAdmin ? '右下の + ボタンから追加できます' : '管理者がイベントを追加するのを待ちましょう'}
            tone="amber"
          />
        ) : (
      ...
      <Modal visible={modalOpen} transparent animationType="fade" onRequestClose={() => setModalOpen(false)}>
        <View
          style={{
            ...
            // web のみ: キーボード高さ分だけ content box を縮め、sheet をキーボード上端に貼り付ける。
        ...
        >
          <View
            style={{
              ...
              padding: SP['4'],
              // キーボード表示中 (web) は home indicator 用 safe-area を足さない (二重の隙間を防ぐ)。
          ...
          >
            ...
            <ScrollView
              ...
              // flexShrink: 1 — panel が縮んだ時 (web キーボード表示中) に list も縮め、
              // 下端の「追加する」ボタンがキーボードの裏に潜らないようにする (Yoga 既定の shrink は 0)。
              style={{ flexShrink: 1 }}
      ...
      >
        ...
        <View style={{ flex: 1, gap: 4 }}>
          ...
          {event.description.length > 0 && (
            <Text style={[T.small, { color: C.text2 }]} numberOfLines={expanded ? undefined : 2}>
              {event.description}
            </Text>
          )}
          {expanded && event.url && (
            <Text style={[T.small, { color: C.accentLight, fontWeight: '700' }]} numberOfLines={1}>{event.url}</Text>
          )}
        </View>
      </View>
    </Pressable>
