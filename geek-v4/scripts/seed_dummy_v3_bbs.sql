...
-- ============================================================
-- 0. クリーンアップ
--   旧バージョン (タイトル先頭に "[v3] " を付けていた) と
--   新バージョン (プレフィックス無し) の両方を削除する。
--   dummy_v2 ユーザー作の本 seed の既知タイトル群だけ消すので、
--   ユーザー手作りスレには影響しない。
-- ============================================================
delete from public.bbs_threads
where (
  title like '[v3]%'  -- 旧バージョン
  or (
    author_id in (
      select id from auth.users where email like 'dummy_v2_%@geek-seed.example'
    )
    and title in (
      '今日あった一番どうでもいい話、書いてけ',
      '真夜中に書き込む人だけが集うスレ',
      'エルデンリング DLC SHADOW OF THE ERDTREE クリアした人語ろう',
      'Apex Legends シーズン22 環境語ろう',
      'ゼルダの伝説 ティアキン、500時間遊んだ猛者おる？',
      '進撃の巨人 完結から1年、結局あのラストどう思う？',
      '連載中のマンガで「これは10年後に古典になる」と思う作品',
      '邦楽で「死ぬ前にこの1曲」教えて',
      '洋楽好き集まれ、最近のヘビロテ教えて',
      '推し活で月いくら使ってる？正直に答えて',
      '推しの誕生日のお祝い、みんな何してる？',
      '東京で本当に美味いラーメン屋、忖度なしで',
      '1人で行ける焼肉、神店舗教えてください',
      '初心者コスプレイヤー、最初の衣装どこで買えばいい？',
      'レイヤー同士の交流イベント、おすすめ教えて',
      '2026年、日本で一番ヤバいニュースなに？',
      'AIが仕事奪う説、自分の業界はどうなる？'
    )
  )
);
...
begin
  ...
  -- ゲーム #1
  ----------------------------------------------------------------
  title := 'エルデンリング DLC SHADOW OF THE ERDTREE クリアした人語ろう';
  category := 'ゲーム';
  reps := array[
  ...
  -- ゲーム #2
  ----------------------------------------------------------------
  title := 'Apex Legends シーズン22 環境語ろう';
  category := 'ゲーム';
  reps := array[
  ...
  -- ゲーム #3
  ----------------------------------------------------------------
  title := 'ゼルダの伝説 ティアキン、500時間遊んだ猛者おる？';
  category := 'ゲーム';
  reps := array[
  ...
  -- マンガ #1
  ----------------------------------------------------------------
  title := '進撃の巨人 完結から1年、結局あのラストどう思う？';
  category := 'マンガ';
  reps := array[
  ...
  -- マンガ #2
  ----------------------------------------------------------------
  title := '連載中のマンガで「これは10年後に古典になる」と思う作品';
  category := 'マンガ';
  reps := array[
  ...
  -- コスプレ #1
  ----------------------------------------------------------------
  title := '初心者コスプレイヤー、最初の衣装どこで買えばいい？';
  category := 'コスプレ';
  reps := array[
...
-- ============================================================
-- 本 seed で作ったスレッドの replies_count + last_reply_at を集計
-- (識別は dummy_v2 ユーザー + 既知タイトル群)
