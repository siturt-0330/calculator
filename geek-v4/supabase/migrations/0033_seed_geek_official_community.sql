...
--
-- 0024 (communities_set_created_by) と 0026 (community_members_normalize_insert)
-- は auth.uid() を要求するため、seed 中は disable して全 INSERT が終わってから
-- 再 enable する。
-- ============================================================
...
begin
  ...
  if v_admin_user_id is null then
    raise notice '[0033] siturt0330@gmail.com not found — skipping';
    return;
  end if;
  ...
  if v_community_id is not null then
    raise notice '[0033] Geek公式 already exists (id=%) — skipping', v_community_id;
    return;
  end if;

  -- ★ auth.uid() を要求する 2 つの trigger をまとめて無効化
  ...
  begin
    -- 1) communities INSERT (handle_new_community trigger 経由で
    --    community_members にも owner が自動追加される)
    ...
    -- 2) 念のため: handle_new_community が動かなかった場合の保険 INSERT
    insert into public.community_members (community_id, user_id, role)
    values (v_community_id, v_admin_user_id, 'owner')
    on conflict (community_id, user_id) do nothing;

    -- 3) タグ
    insert into public.community_tags (community_id, tag) values
      (v_community_id, '公式'),
      (v_community_id, 'お知らせ'),
  ...
  -- ★ trigger を元に戻す (全 INSERT が終わってから)
  alter table public.communities       enable trigger communities_set_created_by;
  alter table public.community_members enable trigger community_members_normalize_insert;
end $$;
